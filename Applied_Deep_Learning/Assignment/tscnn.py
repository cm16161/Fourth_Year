...
def getLoaderPair(source):
    lmcLoader = abstractedDataLoader(source, 'LMC')
    mcLoader = abstractedDataLoader(source, 'MC')
 
    return lmcLoader, mcLoader
...
def runSingleModelInstance(training, test, args, filename):
    model = CNN(height=85, width=41, channels=1, class_count=10, dropout=args.dropout)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=args.learning_rate, weight_decay=5e-3, betas=(0.9,0.9))
...
def main(args):
    ...
    lmcTrain, mcTrain = getLoaderPair('UrbanSound8K_train.pkl')
    lmcTest, mcTest = getLoaderPair('UrbanSound8K_test.pkl')

    runSingleModelInstance(lmcTrain, lmcTest, args, 'lmcdata')
    print("Model One complete, initialising model 2")
    runSingleModelInstance(mcTrain, mcTest, args, 'mcdata')

    with open('lmcdata', 'rb') as f:
        lmcTable = pickle.load(f)
    
    with open('mcdata', 'rb') as f:
        mcTable = pickle.load(f)
    
    with open('ground', 'rb') as f:
        groundTable = pickle.load(f)

    combined = {}
    audioFileNames = mcTable.keys()

    for audio in audioFileNames:
        combined[audio] = [sum(x) for x in zip(mcTable[audio], lmcTable[audio])]
    
    combinedAccuracy = (customAccuracy(combined, groundTable))

    print(f"combined accuracy is: {combinedAccuracy * 100:2.2f}")

    # assert that the same files are in all 3 of the files

    print(len(groundTable))
    print(len(mcTable))
    print(len(lmcTable))
