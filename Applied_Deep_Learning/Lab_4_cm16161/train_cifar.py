...
parser.add_argument("--data-aug-random-perspective", action="store_true")
...
def main(args):
    if args.data_aug_random_perspective:
        if args.data_aug_hflip :
            transform_train = transforms.Compose([
                transforms.RandomHorizontalFlip(),
                transforms.RandomPerspective(),
                transforms.ColorJitter(args.data_aug_brightness),
...
class CNN(nn.Module):
    def __init__(self, height: int, width: int, channels: int,
        ...
        ## TASK 2-1: Define the second convolutional layer and
        ...
        ## TASK 5-1: Define the first FC layer and initialise its
        ## parameters
        self.fc1 = nn.Linear(4096, 1024)
        self.initialise_layer(self.fc1)
        ## TASK 6-1: Define the last FC layer and initialise its
        ## parameters
        self.fc2 = nn.Linear(1024,10)
        self.initialise_layer(self.fc2)
        self.batch2d_1 = nn.BatchNorm2d(
            num_features = 32
        )
        self.batch1d = nn.BatchNorm1d(
            num_features = 1024
        )
        self.batch2d_2 = nn.BatchNorm2d(
            num_features = 64
        )

    def forward(self, images: torch.Tensor) -> torch.Tensor:

        x = F.relu(self.conv1(self.dropout(images)))
    ...
    def train(
        ...
        for epoch in range(start_epoch, epochs):
            ...
            for batch, labels in self.train_loader:
                ...
                ## TASK 12: Step the optimizer and then zero out the
                ## gradient buffers.
                self.optimizer.step()
...
def get_summary_writer_log_dir(args: argparse.Namespace) -> str:
    ...
    tb_log_dir_prefix = (
      ...
      ("random_perspective_" if args.data_aug_random_perspective else "") +
      f"run_"
  )
