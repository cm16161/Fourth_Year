...
#define kp_left 0.1
#define ki_left 0.00000000
#define kd_left 10

#define kp_right 0.1
#define ki_right 0.00000000
#define kd_right 10
...
PID right_vel_pid( kp_right, ki_right, kd_right );
...
void goHome(Kinematics kinematics) {
  ...
  if (!g_move_rotate) {
    ...
    if (home_theta_degrees + 180 - kinematics.m_theta <= 0.0001) {
      stopMotor();
    ...
    if (home_distance * ONE_REVOLUTION / CIRCUMFERENCE - count_right <= 15) {
      stopMotor();
      g_state = at_home;
    }
    ...
    if (output_r <= 15) {
      ...
      moveMotor(LEFT, 30 + 2.2 * head_output);
      moveMotor(RIGHT, 30 - 2 * head_output);
